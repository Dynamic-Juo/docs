...
| 영상 전체 AI 생성 탐지 | CPU 또는 GPU | 선정할 탐지 모델 | 탐지 경로와 모델 미정 | 얼굴 기반 모델과의 자원 공유 여부와 별도 실행 경로 |
...
`근거 부족`과 검증할 주장 없음은 실행 실패로 처리하지 않는다. 얼굴 기반 탐지, 영상 전체 AI 생성 탐지 또는 주장 사실성 검증 중 일부가 실패해도 가능한 다른 분석을 계속한다.
