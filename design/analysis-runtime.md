...
| --- | --- | --- | --- | --- |
| URL 확인·영상 다운로드 | 네트워크, 메모리 또는 임시 디스크 | 수집 도구와 저장 방식 미정 | 네트워크 접근 필요. 저장 방식 미정 | YouTube 응답 시간, 동시 다운로드 제한, 저장 방식, 다운로드 실패와 임시 용량 |
| 프레임·음성 추출 | CPU·메모리, 임시 디스크 | FFmpeg | 사용 후보. 실행 구성 미정 | 작업 수에 따른 CPU·메모리 사용량과 임시 파일 정리 |
| 음성 인식 | CPU 또는 GPU | 선정할 음성 인식 모델 | 모델과 실행 장치 미정 | 한국어 인식 품질, 처리 시간과 메모리 사용량 |
| 얼굴 합성·변형 탐지 | GPU 우선 검토, CPU 실행 가능 여부 확인 | 선정할 탐지 모델 | 모델 선정·테스트 중 | 모델별 실행 장치, GPU 메모리, 입력 프레임 수와 처리 시간 |
| 업로더 AI·합성 표기 확인 | CPU | 제목·설명 텍스트 검사 | 영상 전체 AI 생성 탐지는 MVP에서 제외 | 도입 시 얼굴 기반 모델과의 자원 공유 여부와 별도 실행 경로 |
| 주장 추출·근거 판정 | 네트워크 또는 자체 실행 자원 | 외부 LLM API 또는 자체 모델 | 모델과 제공 방식 미정 | 토큰 사용량, 호출 제한, 응답 시간, 비용과 실패 처리 |
| 근거 검색 | 네트워크 | 외부 검색 API 또는 직접 검색 | 검색 수단 미정 | 주장 수에 따른 호출량, 호출 제한, 응답 시간과 비용 |
| 작업 상태·결과 저장과 폴링 | API 서버, 데이터 저장소 | 구체적인 저장 기술 미정 | 폴링 구조 결정. 저장 기술 미정 | 폴링 주기, 동시 조회 수와 작업 데이터 만료 |
...
be cf550f9의 기본값은 영상 Worker 1개와 주장 Worker 3개다. 영상 A를 처리하는 동안 영상 B는 대기한다. A에서 추출한 주장 1·2·3은 각각 검색·판정을 병렬 실행하고, 하나가 끝나면 다음 주장을 시작한다. 각 주장 안에서 네이버·위키백과 등 제공자 호출은 순차이며 제공자별 호출 제한도 적용한다. 따라서 영상 3건이나 한 주장의 모든 API가 동시에 실행된다는 뜻이 아니다.

```mermaid
flowchart TD
    Q["영상 A 실행 · 영상 B 대기"] --> E["A의 주장 추출"]
    E --> C1["주장 1: 제공자 순차 검색 → 판정"]
    E --> C2["주장 2: 제공자 순차 검색 → 판정"]
    E --> C3["주장 3: 제공자 순차 검색 → 판정"]
    C1 --> NEXT["빈 슬롯에 다음 주장 배정"]
    C2 --> NEXT
    C3 --> NEXT
```
...
| `partially_completed` | 일부 주장 또는 분석 결과를 제공했으며 나머지를 처리 중이다. | 모든 처리가 끝나면 `completed`, 일부 분석을 끝내지 못하면 `completed_with_limitations`, 결과를 만들 수 없으면 `failed`, 분석 시간 상한을 넘으면 `timed_out`으로 전환 |
| `completed` | 지원 범위의 모든 분석이 끝났다. | 최종 상태 |
| `completed_with_limitations` | 일부 분석을 수행하지 못했지만 제공할 결과가 있으며 더 진행할 작업이 없다. | 최종 상태. 완료 결과와 분석하지 못한 범위·이유를 함께 반환 |
...
`근거 부족`과 검증할 주장 없음은 실행 실패로 처리하지 않는다. 얼굴 기반 탐지와 주장 사실성 검증 중 하나가 실패해도 다른 분석을 계속한다.
...
전체 분석이 5분을 넘겨도 최대 10분까지 계속한다. 10분을 초과하면 작업을 `timed_out`으로 전환하고 완료된 결과를 제공하며, 끝나지 않은 주장은 `시간 초과`로 표시한다. 2026-09-08에 공유한 `evaluations`와 API 연동 후의 전체 파이프라인 측정 결과를 바탕으로 이 기준을 조정한다.
...
과거 시간 기록은 [구현 노트 T-3](../evaluations/implementation-notes.md#t-3-자막이-있으면-음성-인식-시간이-0초가-된다-2026-09-07)와 [T-12](../evaluations/implementation-notes.md#t-12-주장-검증-소요-시간-2026-09-07)에 있다. 최신 배포 환경의 재측정·목표 조율은 아직 완료하지 않았다. 과거 자막·규칙 기반·LLM 경로의 수치를 현재 배포 성능으로 합치지 않는다.
...
실행 장비·요청당 비용·결과 보관·대기열·재시도·병렬 검색·실행 자원 검증은 [프로젝트 계획의 미결 표](../project/project-plan.md#미결-사항과-일정-확인)에서 관리한다. 이 문서에 별도 작업 표를 만들지 않는다.
