...
같은 영상의 반복 요청에는 YouTube 영상 ID와 분석 파이프라인 버전을 캐시 키로 사용한다. 유효한 결과가 있으면 분석 ID와 분석 시각을 포함해 반환하며, 사용자가 `다시 분석`을 요청하면 캐시를 건너뛴다. 캐시 결과를 반환하기 전에는 영상의 공개·삭제 상태를 다시 확인한다.

YouTube 영상은 새 영상으로 교체하면 URL이 바뀌지만 기존 URL을 유지한 채 구간 자르기·흐리기·오디오 변경이 가능하므로, 영상 ID만으로 분석 결과를 무기한 재사용하지 않는다. 콘텐츠 해시를 확인하려면 영상을 다시 내려받아야 해 캐시 응답의 속도 이점이 줄어든다. MVP에서는 유효기간을 둔 캐시와 강제 재분석 수단을 사용한다. 정확한 유효기간과 저장 기간은 처리 시간·비용 기준과 함께 정한다.
...
분석 요청 접수 후 2초 이내에 진행 상태를 표시하고, 분석 시작 후 1분 이내에 첫 주장 검증 결과를 반환하며, 전체 결과를 5분 이내에 완료하는 것을 잠정 목표로 한다. 5분을 넘으면 최대 10분까지 처리를 계속한다. 10분을 초과하면 작업을 종료하되 완료된 결과는 보존하고, 끝나지 않은 주장은 `시간 초과` 상태로 반환한다.

2026년 9월 8일 공유할 `evaluations`의 미디어 조작 모델 실행 시간과 API 연동 후 측정할 전체 파이프라인 시간을 구분한다. 다음 항목을 같은 입력 영상과 실행 환경에서 각각 측정해 병목과 전체 소요 시간을 확인한다.
...
각 측정에는 영상 길이, 추출한 프레임 수, 검증할 주장 수, 사용 모델·API, 실행 환경과 캐시 사용 여부를 함께 기록한다. 측정 결과를 바탕으로 1분·5분·10분 기준을 조정한다.
