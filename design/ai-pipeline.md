...
TODO: 음성 합성과 얼굴이 없는 AI 생성 영상의 별도 탐지 경로를 검토한다. [PRD의 추가 기획](../project/prd.md#함께-검토할-사항)에서 지원 범위를 정한 뒤, 필요한 입력 데이터, 탐지 기술 후보와 기존 파이프라인 연결 방식을 설계한다.
...
자막의 사용 가능 기준, 음성 인식 실패 처리, 검증할 주장 선정과 근거 충분성 기준은 미정이다. 근거가 확보되어도 서로 충돌하거나 결론을 내리기 어려우면 판단을 유보하는 방안을 제안한다. 검색 결과가 없다는 이유만으로 주장을 거짓으로 판정하지 않는다.
...
| 주장 추출 | 텍스트·시간 정보 → 검증할 주장·발언 위치 | 사용할 모델, 주장 수, 문맥·시점 보존, 검증 대상 선정 기준 |
| 근거 검색 | 주장 → 관련 근거 문서 | 한국어 검색 수단, 원문 확보, 검색 범위와 날짜 조건 |
| 근거 비교 | 주장·근거 → 지지·반박·근거 부족 결과 | 판단 기준, 서로 충돌하는 근거, 인용과 원문의 일치 여부 |
| 결과 전달 | 두 분석 결과 → 사용자에게 표시할 정보 | 부분 성공·실패 처리, 표시 문구, 결과 데이터 형식 |
