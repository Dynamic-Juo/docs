...
지원 조건은 두 단계로 확인한다. 프런트엔드는 URL 형식과 영상 존재 여부를 분석 요청 전에 확인하고, 조건에 맞지 않으면 요청을 보내지 않는다. Shorts 여부, 공개 상태와 길이는 서버가 요청 직후 확인하고, 주 사용 언어는 음성 인식 이후에 확인한다. 확인할 수 있는 위치가 조건마다 다르므로 같은 지원 조건이라도 안내 시점이 달라진다. 주 사용 언어는 분석을 거절하는 조건이 아니라 판정 신뢰도에 영향을 주는 조건이므로, 확인 결과를 분석 중단이 아닌 안내로 표시한다.
