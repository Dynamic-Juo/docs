...
```mermaid
flowchart TD
    INPUT["YouTube 영상 URL 입력"] --> QUEUED["대기 중"]
...
분석 대상 영상의 썸네일, 제목과 채널명을 결과 화면에 표시한다. 세 값은 [YouTube oEmbed](https://oembed.com/)로 확보하며 API 키가 필요하지 않다. 프런트엔드가 URL을 받는 즉시 표시하고 분석 시작을 기다리지 않는다.

영상 길이와 게시일은 YouTube Data API 키가 있어야 확보할 수 있어 표시하지 않는다.
...
발언이 일정 구간에 걸치면 시작·종료 시점을 표시하고 해당 위치로 이동할 수 있게 한다. 이동은 영상의 해당 시점을 새 탭에서 여는 방식으로 제공한다. 결과 화면을 벗어나면 폴링으로 도착하는 나머지 카드를 놓치기 때문이다. 음성 인식으로 추출한 발언인지 구분한다. 정확한 위치를 확보하지 못하면 `약 00:18` 또는 `발언 위치 확인 불가`처럼 한계를 표시한다.

같은 의미로 병합한 주장은 언급 위치를 모두 표시한다. 대표 위치도 그중 하나이므로 따로 떼어 세지 않는다. 시각을 확보하지 못한 언급은 표시하지 않는다.

주장 원문과 함께 영상에서 실제로 한 말과 그 앞뒤 문맥을 표시한다. 주장 원문은 서버가 다듬은 문장이라, 발언 그대로와 앞뒤 상황을 함께 봐야 사용자가 판정을 검토할 수 있다.

처리 우선순위나 처리 순서는 카드에 표시하지 않는다. 카드는 서버가 준 순서 그대로 놓이고 위에서부터 처리되므로 순서는 카드 위치로 드러난다. 서버가 순번을 주지 않아 화면에서 배열 순서로 매기면 서버가 순서를 바꿀 때 번호가 함께 바뀐다.

완료되지 않은 카드에는 판정이나 근거가 준비된 것처럼 표시하지 않는다. 폴링 응답이 도착하면 카드 상태를 먼저 갱신하고, 제공 가능한 판정·발언 위치·근거·출처를 추가한다.

카드는 판정 근거와 출처를 접어 둔다. 처리 상태, 검증 판정, 주장 원문, 발언 위치까지는 항상 보이고 나머지는 사용자가 펼칠 때 표시한다. 주장이 여러 개인 영상에서 출처까지 모두 펼치면 좁은 화면에서 어떤 주장이 있었는지 훑을 수 없다. 펼침은 카드 안에서 일어나며 상세 화면으로 이동하지 않는다. 보여줄 내용이 없으면 펼치기를 만들지 않는다.

영상 전체 발언문은 제공하지 않는다. 결과를 제공하는 단위는 주장 카드이며, 각 카드에 주장 원문과 발언 위치를 표시한다.

## 미디어 조작 결과

미디어 조작 결과는 주장 카드와 분리된 영역에 표시한다. 분석 중에는 현재 상태를 표시하고 결과가 준비되면 즉시 갱신한다. 주장 검증이 끝날 때까지 미디어 조작 결과 표시를 미루지 않는다.

얼굴 합성·변형 탐지 결과를 다음 네 단계로 표시한다.
...
`조작되지 않음`이나 `실제 영상`처럼 조작이 없다고 단정하는 표현은 사용하지 않는다. 색상은 문구를 대신하지 않는 보조 수단으로 사용하며 단계별 색상과 상태 칩 아이콘은 UI 시안에서 정한다.

영상 전체 AI 생성 탐지는 MVP에서 제외한다. 대신 업로더가 제목이나 설명에 적은 AI·합성 표기를 읽어 `업로더 AI·합성 표기` 항목으로 전달한다. 이 항목은 분석 결과가 아니므로 위 네 단계를 쓰지 않고 `표기 있음`으로만 적는다. 표기가 있을 때만 항목을 만들며, `표기 없음`을 적으면 AI로 만들지 않았다는 뜻으로 읽힌다.

분석이 끝난 뒤에도 결과가 도착하지 않은 탐지는 `분석 불가`로 표시한다. 진행 중 표시를 그대로 두면 끝난 분석을 계속 기다리게 된다.

조작 의심 구간을 타임라인이나 프레임 이미지로 보여주는 것은 MVP에서 제외한다. 프레임 이미지를 제공하는 경로가 없고, 내부 판정 신호는 사용자용 점수로 표시하지 않기로 했다. 어느 구간에서 징후가 나왔는지는 글로 적는다.
...
분석 시작 후 10분을 넘으면 완료된 주장과 미디어 조작 결과를 유지한다. 끝나지 않은 주장 카드는 `시간 초과`로 전환하고, 최종 요약에 완료·미완료·시간 초과 수를 구분한다. 실패·부분 결과와 다시 분석 기준은 아래 항목을 따른다.
...
| 상황 | 전체 처리 | 사용자 표시 |
| --- | --- | --- |
| 지원하지 않는 URL | 분석 시작 안 함 | 지원 조건 안내 |
| 길이 초과·실시간 방송 | 분석 시작 뒤 중단 | 지원 조건 안내 |
| 일부 공개·비공개·연령 제한 영상 | 분석 시작 안 함 | 접근할 수 없는 영상임을 안내 |
| 영상 다운로드 실패 | 분석 중단 | 영상 확보 실패와 다시 시도 안내 |
| 얼굴을 찾지 못함 | 주장 검증 계속 | 얼굴 합성·변형 `분석 불가` |
| 얼굴 탐지 모델 오류 | 가능한 다른 분석 계속 | 해당 탐지만 `분석 불가` |
| 업로더 표기를 읽지 못함 | 얼굴 합성·변형과 주장 검증 계속 | 표기 항목을 만들지 않음 |
| 음성 인식을 사용할 수 없음 | 미디어 분석 계속 | 주장 사실성 검증 `분석 불가` |
| 주 사용 언어가 한국어가 아님 | 모든 분석 계속 | 판정 신뢰도가 떨어질 수 있다는 안내 |
...
지원 조건은 두 단계로 확인한다. 프런트엔드는 URL 형식과 영상 존재 여부를 분석 요청 전에 확인하고, 조건에 맞지 않으면 요청을 보내지 않는다. 공개 상태와 길이는 서버가 요청 직후 확인하고, 주 사용 언어는 음성 인식 이후에 확인한다. 확인할 수 있는 위치가 조건마다 다르므로 같은 지원 조건이라도 안내 시점이 달라진다. 주 사용 언어는 분석을 거절하는 조건이 아니라 판정 신뢰도에 영향을 주는 조건이므로, 확인 결과를 분석 중단이 아닌 안내로 표시한다.

주 사용 언어는 음성 인식으로 확인하므로 분석을 시작하기 전에 거를 수 없다. 한국어가 아닌 영상도 분석을 중단하지 않고, 결과에 판정 신뢰도가 떨어질 수 있다는 안내를 함께 표시한다. 확보한 결과를 버리지 않고 한계를 알리는 방식이다.

주장 하나만 실패한 경우에는 그 카드만 다시 검증하는 기능을 제공한다. 나머지 주장은 이미 끝났으므로 전체를 다시 돌리면 끝난 검증까지 다시 하게 된다. 백엔드가 개별 재검증 경로를 만든 뒤 화면을 붙인다.
...
제공할 결과가 하나도 없이 `failed`로 끝난 경우에는 최종 요약을 만들지 않는다. 집계할 결과가 없는데 판정 수를 0으로 나열하면 판정을 마친 것처럼 보인다. 이때는 어느 단계에서 멈췄는지와 분석 ID만 표시한다.

최종 요약에는 다음 정보를 포함한다.

- 미디어 조작 결과
- 근거와 일치·근거와 불일치·근거 부족 주장 수
- 완료·미완료·시간 초과 주장 수
- 전체 분석 상태
- 분석 ID
- 분석 시각
...
| 주장 판정 | `근거와 일치`·`근거와 불일치`·`근거 부족`으로 결정. 판정별 색상과 상태 칩 세부 문구는 UI 시안에서 확인 | U-03 |
| 미디어 조작 표시 | 네 단계 표현 결정. 단계별 색상과 상태 칩 아이콘은 UI 시안에서 확인 | U-04 |
| 실패·부분 결과 | 상태·단계별 계속 처리와 다시 분석 기준 결정. 구체적인 안내 문구는 UI 시안에서 확인 | U-05 |
주장 카드 펼침(U-08), 처리 순번 표시(U-09)와 개별 재검증(U-10)은 시안 작업에서 결정했다. 카드는 접어 두고 순번은 표시하지 않는다. 개별 재검증은 제공하며 백엔드가 경로를 만든 뒤 화면을 붙인다. 내용은 [주장 카드](#주장-카드)와 [실패와 부분 결과](#실패와-부분-결과)에 있다.
