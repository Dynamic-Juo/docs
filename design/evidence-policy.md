...
이 정책은 변경하지 않는다. [PR #7의 시점·충돌 질문](https://github.com/Dynamic-Juo/docs/pull/7#discussion_r3958924934)은 테스트 영상 선택이 아니라 위 판정 절차의 구현 여부에 관한 질문이다.

be cf550f9에는 원문 범위·출처 검증·인용 대조·근거 충분성을 확인하는 서버 조건이 있다. 그러나 기본 검색 제공자는 검색 발췌만 공급하므로 원문 확보를 완료한 것으로 볼 수 없다. 검색 결과를 원문으로 표시해 판정 조건을 통과시키지 않는다. 인용 문자열의 존재만으로 의미상 지지·반박이 검증되는 것도 아니므로, 원문 경로를 붙일 때 조건·수치·시점·충돌의 고정 실패 사례를 함께 확인한다. 구현 증거는 [최신 점검](../evaluations/mvp-implementation-audit.md#2026-09-13-재점검), 완료 조건과 담당자는 [프로젝트 계획](../project/project-plan.md#미결-사항과-일정-확인)에서 관리한다.

인용 검사는 별도 판정 모델이 아니라 백엔드 Python 코드의 `_apply_citations`·`_quote_found_in`에서 수행한다. LLM이 지정한 자료 번호·인용·이유의 형식을 확인하고, 공백·따옴표 등을 정규화한 인용이 바로 그 자료의 전달 텍스트에 포함되는지 대조한다. 너무 짧은 인용은 제외한다. 이어 `_decision_evidence_is_sufficient`가 원문 범위·출처·근거 수 조건을 검사한다. 이는 실제 문자열 존재에 대한 검사이며 의미상 함의·모순을 검증하는 독립 모델은 아니다.
