...
```text
.
├── AGENTS.md                       # 에이전트가 항상 따르는 문서 작성 규칙
...
1. 미디어 조작 탐지: 인물의 얼굴 합성·변형을 분석하고, 업로더가 제목이나 설명에 적은 AI·합성 표기를 함께 전달한다.
...
- 문서 본문은 평서체로 통일한다.
- 표의 각 열에는 하나의 독립된 속성만 기록한다. 상태·시점, 담당자·기한, 근거·결정일처럼 별도로 갱신하는 값은 각각 다른 열에 둔다.
