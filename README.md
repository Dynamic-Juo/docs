...
현재 구현과 배포 상태는 [백엔드 공유 인수인계](operations/backend-handoff.md)에서 확인한다. API 계약과 Swagger 주소, 코드 커밋, 검증 범위, 기존 PR #7과 기획 본문의 차이를 한곳에서 연결한다. PR 댓글의 제안, 코드 구현 완료, 로컬 검증, main 병합과 개발계 배포를 같은 상태로 취급하지 않는다. 문서는 현재 작업 브랜치로만 공유한다. 백엔드 PR #2 병합과 ARM64 CI·이미지 게시는 완료했지만 실제 맥미니 교체는 승인 전이다.

- [중간 점검 기록](evaluations/2026-09-11-midpoint-review.md): 보안·분석·프롬프트 수정과 당시 314개 모의 테스트 결과
- [기획 변경 검토안](rfcs/public-mvp-readiness.md): 미승인 제안과 확인할 자막 정책
- [기존 검토 PR #7](https://github.com/Dynamic-Juo/docs/pull/7): 기존 질문과 리뷰 이력. 답글은 사용자가 별도로 마무리하며, 이번 공유에서는 새 PR 생성·댓글 게시·병합을 하지 않는다.
...
| `operations/` | 공유·연동의 시작점, 배포 여부, 검증 범위와 담당자 간 확인 사항 | API 명세 원문 복사, 승인되지 않은 제품·설계 변경 |
