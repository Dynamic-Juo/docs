...
현재 구현과 배포 상태는 [백엔드 공유 인수인계](operations/backend-handoff.md)에서 확인한다. API 계약과 Swagger 주소, 코드 브랜치, 검증 범위, 기존 PR #7과 기획 본문의 차이를 한곳에서 연결한다. PR 댓글의 제안, 코드 구현 완료, 배포 완료를 같은 상태로 취급하지 않는다.
...
- [기존 검토 PR #7](https://github.com/Dynamic-Juo/docs/pull/7): 기존 질문과 리뷰 이력. 이번 공유는 새 PR·댓글·병합 없이 브랜치에 올린다.
